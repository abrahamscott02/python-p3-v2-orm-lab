...
class Review:
    ...
    def instance_from_db(cls, row):
        ...
        # row[0] is assumed to be the id from the database
        review = cls.all.get(row[0])
        
        if review:
            # If an instance with this ID already exists, update its attributes
            review.year = row[1]
            review.summary = row[2]
            review.employee_id = row[3]
        else:
            # Otherwise, create a new instance and add it to the dictionary
